                                    ...
                                    char *_buffer, int size)
{
  ...
  if (rcvd_count == entity->rx_deliv) {
    ...
    while (entity->rx_list != NULL && count == entity->rx_list->count) {
      ...
      LATSEQ_P("U pdcp.deliver--sdap.sdu", "::sn=%u.PSbuf=%x.pdcpsdusize=%u.pdusessionid=%u", cur->count, cur->buffer, sdu->size, entity->rb_id);
                                      ...
                                      int pdu_max_size)
{
  ...
  LATSEQ_P("D pdcp.hdr--pdcp.int_ciph", "::SPbuf=%u.sn=%u.Pbuf=%x.pdusessionid=%u", buffer, sn, buf, entity->rb_id);
...
static void check_t_reordering(nr_pdcp_entity_t *entity)
{
  ...
  while (entity->rx_list != NULL && entity->rx_list->count < entity->rx_reord) {
    ...
    LATSEQ_P("U pdcp.deliver_ooo--sdap.sdu", "::sn=%u.PSbuf=%x.pdusessionid=%u", cur->count, cur->buffer, entity->rb_id);
    entity->deliver_sdu(entity->deliver_sdu_data, entity,
                        cur->buffer, cur->size,
                        &cur->msg_integrity);
    entity->rx_list = cur->next;
  ...
  while (entity->rx_list != NULL && count == entity->rx_list->count) {
    ...
    LATSEQ_P("U pdcp.deliver_reorder--sdap.sdu", "::sn=%u.PSbuf=%x.pdusessionid=%u", cur->count, cur->buffer, entity->rb_id);
    entity->deliver_sdu(entity->deliver_sdu_data, entity,
                        cur->buffer, cur->size,
                        &cur->msg_integrity);
    entity->rx_list = cur->next;
