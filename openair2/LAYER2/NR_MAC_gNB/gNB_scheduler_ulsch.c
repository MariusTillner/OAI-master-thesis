    ...
    switch (lcid) {
      ...
      case UL_SCH_LCID_DTCH ...(UL_SCH_LCID_DTCH + 28):
        ...
        if (!c || c->suspended) {
          ...
          UE->mac_stats.ul.lc_bytes[lcid] += mac_len;
          LATSEQ_P("U mac.demuxed--rlc.dec", "::fm%u.sl%u.hqpid%u.MRbuf%u.rnti%u.macsdusize%u.UEbuffer%u.10xSNR%u.UL_BLER%f.CSI_RSRP%d", frameP, slot, harq_pid, pduP+mac_subheader_len, UE->rnti, mac_len, sched_ctrl->estimated_ul_buffer - mac_len, sched_ctrl->pusch_snrx10, sched_ctrl->ul_bler_stats.bler, sched_ctrl->CSI_report.csirs_rsrp_report.RSRP);
          nr_mac_rlc_data_ind(module_idP, UE->rnti, true, lcid, (char *)(pduP + mac_subheader_len), mac_len);
