    ...
    switch (lcid) {
      ...
      case UL_SCH_LCID_DTCH ...(UL_SCH_LCID_DTCH + 28):
        ...
        if (!c || c->suspended) {
          ...
          UE->mac_stats.ul.lc_bytes[lcid] += mac_len;
          LATSEQ_P("U mac.demuxed--rlc.dec", "mac_sdu_sz=%u.ue_buf=%d.snr10=%u.ul_bler_pct=%d.rsrp=%d.prbs=%d::fm=%u.sl=%u.hqpid=%u.mrbuf=%x", mac_len, sched_ctrl->estimated_ul_buffer, sched_ctrl->pusch_snrx10, (int32_t)(sched_ctrl->ul_bler_stats.bler * 100), sched_ctrl->CSI_report.ssb_rsrp_report.r[0].RSRP, UE->mac_stats.NPRB, frameP, slot, harq_pid, pduP+mac_subheader_len);
          nr_mac_rlc_data_ind(module_idP, UE->rnti, true, lcid, (char *)(pduP + mac_subheader_len), mac_len);
