...
void post_process_dlsch(gNB_MAC_INST *nr_mac, post_process_pdsch_t *pdsch, NR_UE_info_t *UE, NR_sched_pdsch_t *sched_pdsch)
{
  ...
  if (harq->round != 0) { /* retransmission */
    ...
    LATSEQ_P("D mac.retx--mac.dci", "hqround=%ld.tb_sz=%ld::hqpid=%ld.dl_tb_id=%ld.fm=%ld.sl=%ld", harq->round, TBS, current_harq_pid, harq->dl_tb_id, frame, slot);
    LATSEQ_P("D mac.retx--mac.retx", "hqround=%ld.tb_sz=%ld::hqpid=%ld.dl_tb_id=%ld", harq->round, TBS, current_harq_pid, harq->dl_tb_id);
    LOG_D(NR_MAC,
