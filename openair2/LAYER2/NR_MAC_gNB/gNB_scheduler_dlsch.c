      ...
      for (int i = 0; i < seq_arr_size(&sched_ctrl->lc_config); ++i) {
        ...
        while (bufEnd-buf > sizeof(NR_MAC_SUBHEADER_LONG) + 1 ) {
          ...
          LOG_D(NR_MAC,
                ...
                ndata,
                bufEnd-buf-sizeof(NR_MAC_SUBHEADER_LONG));
          LATSEQ_P("D mac.handover--mac.subhdr", "::RMbuf%u.fm%u.sl%u.fmretx%u.slretx%u.hqpid%u.rnti%u.10xDL_BLER%%%d", (char *)buf+sizeof(NR_MAC_SUBHEADER_LONG), frame, slot, frame, slot, current_harq_pid, rnti, (int32_t)(sched_ctrl->dl_bler_stats.bler * 1000));
