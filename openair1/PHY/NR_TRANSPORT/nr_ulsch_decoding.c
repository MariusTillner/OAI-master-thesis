  ...
  for (uint8_t pusch_id = 0; pusch_id < nb_pusch; pusch_id++) {
    ...
    for (int r = 0; r < TB_parameters.C; r++) {
      ...
      if (nrLDPC_segment_decoding_parameters.decodeSuccess) {
        ...
        LATSEQ_P("U phy.CB_dec--phy.TB_dec", "iter=%u::fm=%u.sl=%u.hqpid=%u.seg=%u.rnti=%u", decode_iter, ulsch->frame, nr_tti_rx, ulsch->harq_pid, r, ulsch->rnti);
      } else {
        LOG_D(PHY, "uplink segment error %d/%d\n", r, harq_process->C);
        LOG_D(PHY, "ULSCH %d in error\n", ULSCH_id);
        if (harq_process->round == 3) {
          LATSEQ_P("U phy.CB_dec--phy.retx_drop", "seg_ct=%u.hqround=%u.iter=%u::fm=%u.sl=%u.hqpid=%u.seg=%u.rnti=%u", TB_parameters.C, harq_process->round, decode_iter, ulsch->frame, nr_tti_rx, ulsch->harq_pid, r, ulsch->rnti);
        } else {
          LATSEQ_P("U phy.CB_dec--phy.dec_fail", "seg_ct=%u.hqround=%u.iter=%u::fm=%u.sl=%u.hqpid=%u.seg=%u.rnti=%u", TB_parameters.C, harq_process->round, decode_iter, ulsch->frame, nr_tti_rx, ulsch->harq_pid, r, ulsch->rnti);
          LATSEQ_P("U phy.dec_fail--phy.prach_pucch", "::hqpid=%u.seg=%u.rnti=%u", ulsch->harq_pid, r, ulsch->rnti);
        }
      }
