                   ...
                   int beam_nb)
{
  ...
  LATSEQ_P("U phy.prach_pucch--phy.CH_est", "mcs=%ld.qm=%ld:rnti=%ld:fm=%ld.sl=%ld.hqpid=%ld", rel15_ul->mcs_index, rel15_ul->qam_mod_order, rel15_ul->rnti, frame, slot, harq_pid);
  int max_ch = 0;
  uint32_t nvar = 0;
  int end_symbol = rel15_ul->start_symbol_index + rel15_ul->nr_of_symbols;
  ...
  LATSEQ_P("U phy.CH_est--phy.demodulated", "ant_rx=%ld.layers=%ld.syms=%ld:rnti=%ld:fm=%ld.sl=%ld.hqpid=%ld", frame_parms->nb_antennas_rx, rel15_ul->nrOfLayers, rel15_ul->nr_of_symbols+1, rel15_ul->rnti, frame, slot, harq_pid);
  ...
  join_task_ans(&ans);
  LATSEQ_P("U phy.demodulated--phy.CB_dec", ":rnti=%ld:fm=%ld.sl=%ld.hqpid=%ld", rel15_ul->rnti, frame, slot, harq_pid);
