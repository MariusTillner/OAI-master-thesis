...
static void rx_rf(RU_t *ru, int *frame, int *slot)
{
  ...
  stop_meas(&ru->rx_fhaul);
  LATSEQ_P("U phy.rx_samples_in--phy.fft", "iq_sz=%ld::fm=%ld.sl=%ld", samples_per_slot*sizeof(c16_t), *frame, *slot);
}
...
void tx_rf(RU_t *ru, int frame,int slot, uint64_t timestamp)
{
  ...
  uint32_t txs = ru->rfdevice.trx_write_func(&ru->rfdevice,
                                             ...
                                             nt,
                                             flags);
  LATSEQ_P("D phy.tx_samples_out--phy.out", "iq_sz=%ld::fm=%ld.sl=%ld", (siglen + sf_extension)*sizeof(c16_t), frame, slot);
  LOG_D(PHY,
...
void ru_tx_func(void *param)
{
  ...
  LATSEQ_P("D phy.ifft--phy.tx_samples_out", "sl_ahead=%ld::fm=%ld.sl=%ld", ru->sl_ahead, frame_tx, slot_tx);
...
void *ru_thread(void *param)
{
  ...
  while (!oai_exit) {
    ...
    LATSEQ_P("U phy.rx_samples_start--phy.rx_samples_in", "::fm=%ld.sl=%ld", frame, slot);
    ...
    if (slot_type == NR_UPLINK_SLOT || slot_type == NR_MIXED_SLOT) {
      ...
      if (ru->feprx) {
        ...
        LATSEQ_P("U phy.fft--phy.prach_pucch", "::fm=%ld.sl=%ld", frame, slot);
